function writeDownloadLink(){
	...
	function add_axis_title(graph,data) { 
		...
		graph_select.append("line") // add line for hdi separation
			.style("stroke", "black")  
			.attr("x1", xScale(11.75*(bar_space+1)))
			.attr("y1", -20)  
			.attr("x2",xScale(11.75*(bar_space+1)))
		...
		graph_select.append("text") // add x axis subtitle
			.attr("class", "y_title")
			.attr("text-anchor", "middle")
			.attr("transform", "translate("+xScale(17*(bar_space+1))+"," +(var_height +150) + ")")
			.text("ЕС-28")
	...
	function add_circle_line(graph, data, color) {
		...
		var node_label = graph_select
			...
			.attr("transform", function(d, i) {
				shift = (d.values[0].values[0].hdi-1)/2
				pos = (d.values[0].values[0].rank)
				return "translate(" + (xScale((pos+shift)*(bar_space+1)+0.5)) + "," + (var_height +10)+ ")";
				})
